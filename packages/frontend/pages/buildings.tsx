            ...
            <div className="hidden md:flex items-center space-x-8">
              {['Home', 'Buildings', 'Portal', 'Contact'].map((item) => (
                <Link key={item} href={item === 'Home' ? '/' : `/${item.toLowerCase()}`}
                  className={`text-sm font-medium tracking-wide transition-colors ${item === 'Buildings' ? 'text-emerald-600' : 'text-slate-600 hover:text-slate-900'}`}>
                  {item}
                </Link>
              ))}
            </div>
            <div className="flex items-center space-x-4">
              {user ? (
                <>
                  ...
                  <button onClick={handleLogout} className="text-sm font-semibold px-4 py-2 rounded-lg text-slate-700 hover:bg-slate-100 transition-all">Logout</button>
                </>
              ) : (
